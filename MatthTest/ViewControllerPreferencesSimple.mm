//
//  ViewControllerPrefencesSimple.mm
...
@implementation ViewControllerPreferencesSimple
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	switch (iSection) {
		case 0:
			...
			switch (iCellNo) {
				case 0:
					sTitleMain = @"Difficulty";
					nSliderMax = [[[NSNumber alloc] initWithFloat:(float)1000.0] autorelease];
					nSliderMin = [[[NSNumber alloc] initWithFloat:(float)10.0] autorelease];
					break;
				case 1:
...
-(void)openQuiz {
//used on the load event but not the solution
	NSString* sPathHome = [[NSString alloc] initWithString:[[[UIApplication sharedApplication] delegate] DirDocumentsPath]];
	std::string stdPathHome = [sPathHome UTF8String];
...
-(void)groupEnabled:(NSString*)sGroup bEnabled:(NSNumber*)bEnabled {
	//do nothing this function is only really useful in the group and subject preference view controllers
};
